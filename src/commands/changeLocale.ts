...
async function changeLocaleCommand(ctx: ICommandContext, isForced = false) {
    if (!isForced && ctx.chat.type !== "private" && !(ctx.from && (await isChatOwner(ctx.chat.id, ctx.from.id)))) {
        return;
    }
