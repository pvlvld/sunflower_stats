...
async function removeFromChatCleanup(ctx: IGroupTextContext): Promise<void> {
  ...
  if (!targetMembers) {
    ...
    switch (await setRestStatus(ctx, targetId)) {
      ...
      case "not enough rights edit admin":
        return void (await ctx.reply(
          `${
            active.data[ctx.chat.id]?.[targetId]?.name
          } адмін, він не може бути видалений під час чистки та не потребує ресту.`
        ));

      case "success":
        return void (await ctx.reply(
          `✅ ${active.data[ctx.chat.id]?.[targetId]?.name} помічено як рест.`
        ));
  ...
  if (targetMembers.length < startLength) {
    ...
    return void (await ctx
      .reply(`✅ ${active.data[ctx.chat.id]?.[targetId]?.name} успішно виключено з чистки.`)
      .catch((e) => {}));
  }
